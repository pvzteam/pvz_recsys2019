...
def feat_extract(df_ori):
    ...
    df_feat = custom_cate_encoding.gen_hist_feat(df, \
            ['session_id'], 'action_type', ratio=False)
    df_feat.columns = ['session_id'] + ['session_hist_{}' \
            .format(val) for val in df_feat.columns.tolist()[1:]]
    print (df_feat.shape)
    print (df_feat.head())
...
if __name__ == "__main__":
    ...
    #gen_fea(base_tr_path, base_te_path)
    gen_fea()
