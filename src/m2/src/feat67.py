...
def feat_extract(df):
    ...
    df['act_sum'] = df[cols].sum(axis=1)
    df_feat = df[ID_NAMES + ['hist_clickout item', 'act_sum']]
    df_feat.columns = ID_NAMES + ['clickout item', 'act_sum']

    df_feat['session_act_sum'] = df_feat.groupby('session_id') \
            ['act_sum'].transform('sum')
...
def add_meta_fea(df):
    for op in ['max', 'min', 'median']:
        df['prices_sub_impressions_by_prices_{}'.format(op)] = \
                df['prices'] - df['impressions_by_prices_{}'.format(op)]
        df['prices_div_impressions_by_prices_{}'.format(op)] = \
                df['prices'] / df['impressions_by_prices_{}'.format(op)]

    for op in ['median']:
        df['impr_rank_sub_impressions_by_impr_rank_{}'.format(op)] = \
                df['impr_rank'] - df['impressions_by_impr_rank_{}'.format(op)]
...
if __name__ == "__main__":
    ...
    root_path = '../feature/'
    base_tr_path = root_path + 'tr_s0_41.ftr'
    base_te_path = root_path + 'te_s0_41.ftr'
