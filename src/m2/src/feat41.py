...
def feat_extract(df):
    df_rank_1 = df[ID_NAMES + ['impressions_target']] \
            .drop_duplicates(subset='session_id', keep='first')
    df_rank_1.columns = ID_NAMES + ['impr_rank_1_impressions_target']

    df_feat = df[ID_NAMES + ['impressions_target']]
    df_feat = cate_encoding.cate_num_stat(df, df_feat, \
            ['session_id'], 'impressions_target', ['min', 'max', 'median', 'std'])
    for op in ['min', 'max', 'median']:
        df_feat['impressions_target_sub_session_{}'.format(op)] = \
                df_feat['impressions_target'] - \
                df_feat['session_id_by_impressions_target_{}'.format(op)]
    
    df_feat = df_feat.merge(df_rank_1, on=ID_NAMES, how='left')
    df_feat['impressions_target_sub_impr_rank_1_impressions_target'] = \
            df_feat['impressions_target'] - df_feat['impr_rank_1_impressions_target']

    del df_feat['impressions_target']
...
def gen_fea(base_tr_path=None, base_te_path=None):
    ...
    tr = loader.load_df('../feature/tr_fea_s0_35.ftr')
    te = loader.load_df('../feature/te_fea_s0_35.ftr')
...
if __name__ == "__main__":
    ...
    root_path = '../feature/'
    base_tr_path = root_path + 'tr_s0_38.ftr'
    base_te_path = root_path + 'te_s0_38.ftr'
