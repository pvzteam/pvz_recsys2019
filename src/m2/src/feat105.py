...
def feat_extract(df):
    df_rank1 = df[df.impr_rank == 0].drop_duplicates(subset=['session_id'])
    df_rank1 = df_rank1[['session_id', 'impressions']]
    df_rank1.columns = ['session_id', 'item_rank_1']
    df = df.merge(df_rank1, on='session_id', how='left')

    df_sim = loader.load_df('../../../feat/m1_similarity_all.ftr')
    #df_sim.rename(columns={'item_id': 'impressions', \
    #        'item_id_anchor': 'item_rank_1'}, inplace=True)
    df_sim.columns = ['impressions', 'item_rank_1'] + \
            ['rank_1-{}'.format(c) for c in df_sim.columns.tolist()[2:]]
    df_feat = df[ID_NAMES + ['item_rank_1']].merge(df_sim, \
            on=['impressions', 'item_rank_1'], how='left')
...
if __name__ == "__main__":
    ...
    root_path = '../feature/'
    base_tr_path = root_path + 'tr_s0_87.ftr'
    base_te_path = root_path + 'te_s0_87.ftr'
