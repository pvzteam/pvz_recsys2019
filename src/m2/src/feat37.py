...
def feat_extract(df, acts, prefix):
    ...
    df_feat['impressions_{}_ratio'.format(prefix)] = \
            df_feat.groupby('impressions')['actived'].transform('mean')
    df_feat = df_feat[ID_NAMES + ['impressions_{}_ratio'.format(prefix)]]
