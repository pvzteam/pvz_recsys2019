...
def extract_list_cnt(df):
    ...
    for i in range(1,5):
        df['rank_%d' % i] = df['rank_%d' % (i-1)] + '_' + df['rank_%d' % i]
    ...
    for i in range(5):
        ...
        df_cnt.rename(columns={'count':'rank_%d_cnt' % i,'nunique':'rank_%d_uniq' % i},inplace=True)
        cols = cols + ['rank_%d_cnt' % i,'rank_%d_uniq' % i]
...
extract(tr_out, config.feat+'m3_tr_imprlist_feat.ftr')
extract(te_out, config.feat+'m3_te_imprlist_feat.ftr')
